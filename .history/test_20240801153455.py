...
def get_recommendation(model, user_id, n=5):
    ...
    if n <= len(next_books):
        ...
        # Always return books sorted by their predicted rating
        recommended_books = Books[Books.ISBN.isin(book_ids)][['ISBN', 'title', 'author', 'average_rating', 'count_ratings', 'image']]
        recommended_books = recommended_books.set_index('ISBN').reindex(book_ids).reset_index()
