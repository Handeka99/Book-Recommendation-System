...
def display_books(books):
    for _, row in books.iterrows():
        ...
        <div style="display: flex; align-items: center; margin-bottom: 20px;">
            <img src="{row['image_url']}" alt="{row['title']}" style="width: 100px; height: 150px; margin-right: 20px;">
            <div>
                <h4>{row['title']}</h4>
                <p><b>Author:</b> {row['author']}</p>
                <p><b>Average Rating:</b> {row['average_rating']}</p>
                <p><b>Number of Ratings:</b> {row['count_ratings']}</p>
            </div>
        </div>
        """, unsafe_allow_html=True)
