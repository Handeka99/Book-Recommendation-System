...
def get_recommendation(model, user_id, n=5):
    ...
    if n <= len(next_books):
        ...
        ratings = sorted(ratings, key=lambda x: x[1], reverse=True)
        book_ids = [id for id, rate in ratings[:n]]
        return Books[Books.ISBN.isin(book_ids)][['ISBN', 'title', 'author', 'average_rating', 'count_ratings', 'image_url']]
    else:
        st.write('Please reduce your recommendation request')
        return
...
def get_top_20_books():
    ...
    return top_books[['ISBN', 'title', 'author', 'average_rating', 'count_ratings', 'image_url']]

def display_books(books):
    for _, row in books.iterrows():
        st.markdown(f"""
        <div style="display: flex; align-items: center; margin-bottom: 20px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2); padding: 10px; border-radius: 10px; background-color: white;">
            <img src="{row['image_url']}" alt="{row['title']}" style="width: 100px; height: 150px; margin-right: 20px; border-radius: 5px;">
            <div>
