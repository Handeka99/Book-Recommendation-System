...
# Shuffle the ratings for unbiased result
all_ratings = list(data.raw_ratings)
random.shuffle(all_ratings)
...
# Initialize algorithms
npred = NormalPredictor()
knn = KNNBasic(sim_options={"name": "cosine", "user_based": True}, random_state=my_seed)
svd = SVDpp(random_state=my_seed)
svdpp = SVDpp(random_state=my_seed)
...
def train_models():
    ...
    try:
        ...
        if trainset.n_ratings == 0:
            print("Training set is empty.")
            return
        
        print("Training NormalPredictor...")
        npred.fit(trainset)
