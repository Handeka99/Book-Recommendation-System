...
def get_recommendation(model, user_id, n=5):
    ...
    if n <= len(next_books):
        ...
        ratings = sorted(ratings, key=lambda x: x[1], reverse=True)
        book_ids = [id for id, rate in ratings[:n]]
        recommend_books = Books[Books.ISBN.isin(book_ids)][['ISBN', 'title', 'author', 'average_rating', 'count_ratings', 'image']]
        recommend_books['average_rating'] = recommended_books['average_rating'].round(1)
...
def get_top_20_books():
    ...
    top_books['average_rating'] = top_books['average_rating'].round(1)
