...
def get_recommendation(model, user_id, n=5):
    ...
    if n <= len(next_books):
        ...
        #recommend_books = Books[Books.ISBN.isin(book_ids)][['ISBN', 'title', 'author', 'year', 'average_rating', 'count_ratings', 'predicted_rating', 'image']]
        recommended_books = Books[Books.ISBN.isin(book_ids)].set_index('ISBN').loc[book_ids].reset_index()
...
def get_top_20_books():
    ...
    return top_books[['ISBN', 'title', 'author', 'year', 'average_rating', 'count_ratings', 'predicted_rating', 'image']]
    ...
    for _, row in books.iterrows():
        ...
        <div style="display: flex; align-items: center; margin-bottom: 20px; box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2); padding: 10px; border-radius: 10px; background-color: #134B70;">
            ...
            <div>
                ...
                <p style="margin: 0; padding: 0;"><b>Average Rating:</b> {row['predicted_rating']}</p>
