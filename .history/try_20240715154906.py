...
# Function to create card item for book display
def create_card_item(book_name, poster_url, year, rating):
    card_css = """
    <style>
    .card-container {
        padding: 10px;
...
# Main page
if options == 'Home':
    st.markdown("<h1 style='text-align: center; color: black;'>Welcome to the Book Recommender System</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color:black;'>Use the navigation bar to explore top books and get recommendations.</p>", unsafe_allow_html=True)
    ...
    if st.button("Recommend"):
        ...
        for book, poster in zip(recommended_books, poster_urls):
            ...
            create_card_item(book, poster, book_data['year'], book_data['rating'])
